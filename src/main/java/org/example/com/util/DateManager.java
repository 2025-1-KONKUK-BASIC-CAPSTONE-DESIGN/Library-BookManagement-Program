...
public class DateManager {
    ...
    private static final String LOAN_DATE_FILE_PATH = System.getProperty("user.home") + "/loan_date.txt";
    ...
    public static boolean saveLoanDateToFile(Date date) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(LOAN_DATE_FILE_PATH))) {
            writer.write(date.getValue());
            return true;
        } catch (IOException e) {
            return false;
    ...
    public static Date loadLoanDateFromFile() {
        File file = new File(LOAN_DATE_FILE_PATH);
        if (!file.exists()) return null;
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            String line = reader.readLine();
            return Date.parse(line);
